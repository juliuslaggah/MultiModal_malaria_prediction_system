...
LOG_CSV_PATH = os.path.join(LOG_DIR, f"{config.get('experiment_name','CNN')}_efficientnetb0_{run_id}.csv")

# Set Device
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"✅ Device: {device}")

# --- CUSTOM UTILS FOR ROBUSTNESS (MATCHING OTHERS) ---
class AddGaussianNoise(object):
    """
    Adds Gaussian Noise to simulate low-quality sensor grain.
    Matches High Severity Standard (std=0.1).
    """
...
print(f"🔧 Configuring HIGH SEVERITY Pipeline (EfficientNet):")
...
# FIXED SEED 42 ENSURES IDENTICAL SPLIT
...
def train():
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            save_path = os.path.join(CHECKPOINT_DIR, "efficientnet_b0_best.pth")
    ...
    print(f"\n🏆 Training Complete. Best EfficientNet Acc: {best_acc:.2f}%")
