...
TEST_JSON_PATH = os.path.join(current_dir, "test_images.json")
...
LOG_CSV_PATH = os.path.join(LOG_DIR, f"{config.get('experiment_name','CNN')}_mobilenetv2_{run_id}.csv")

# Set Device
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"✅ Device: {device}")

# --- CUSTOM UTILS FOR ROBUSTNESS ---
class AddGaussianNoise(object):
    """
    Adds Gaussian Noise to the tensor to simulate low-quality sensor grain.
    """
    def __init__(self, mean=0., std=0.1):  # Increased std to 0.1 for heavy grain
        self.std = std
        self.mean = mean
...
if os.path.exists(combined_data_path):
    print(f"✅ Found 'Combined_Data' subfolder. Using: {combined_data_path}")
    data_dir = combined_data_path
else:
    print(f"⚠️ 'Combined_Data' subfolder not found. Using raw path: {base_data_dir}")
...
# --- SAVE TEST SET PATHS ---
print("\n📝 Saving Test Set Paths to JSON...")
test_image_paths = []
for idx in test_indices:
    path, _ = eval_base.samples[idx]
    test_image_paths.append(path)

with open(TEST_JSON_PATH, "w") as f:
    json.dump(test_image_paths, f, indent=4)
...
def train():
    ...
    print(f"\n🚀 Starting High-Severity Training for {epochs} epochs...")
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            save_path = os.path.join(CHECKPOINT_DIR, "mobilenet_v2_best.pth")
    ...
    print(f"\n🏆 Training Complete. Best Robust Validation Accuracy: {best_acc:.2f}%")
